...
class Tab3(): #Periferimating kalkulator
    ...
    def radCall(self):
        ...
        if   radSel == 1:
            ...
            self.test = Label(self.peri, text = "Periferimating Yttre Skjærkant:", font=FONT1).grid(row = 6, column = 0, sticky=N)
            #tab1.update()
            ...
            self.test1 = Label(self.peri, text = "Periferimating Indre Skjærkant:", font=FONT1).grid(row = 6, column = 0, sticky=N)
            #tab1.update()
            self.perif = Text(self.peri, width = 5, height = 1)
            self.perif.grid(row = 6, column = 1, columnspan = 2, sticky=W)
